...
async def analyze_time_series(request: TimeSeriesRequest):
    ...
    try:
        ...
        value_series = pd.to_numeric(df[request.value_column], errors='coerce')
        valid_count = value_series.notna().sum()
        if valid_count < 2:
            raise HTTPException(
                status_code=400,
                detail=f"La columna '{request.value_column}' no contiene suficientes datos numéricos válidos ({valid_count} encontrados, mínimo 2)"
            )
        
        # Validar que la columna temporal sea parseable como fecha
        try:
            pd.to_datetime(df[request.time_column])
        except Exception:
            raise HTTPException(
                status_code=400,
                detail=f"La columna '{request.time_column}' no contiene fechas válidas"
            )
